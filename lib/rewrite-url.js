...
module.exports = function(file, config) {
    ...
    .then(file => {
		...
		file.css.walkDecls(function(decl) {
			decl.value = replaceUrl(decl.value, file, config);
		});

		// rewrite @import
		file.css.walkAtRules(function(rule) {
			if (rule.name === 'import') {
				rule.params = replaceUrl(rule.params, file, config);
			}
		});
...
function replaceUrl(str, file, config) {
	return str.replace(/url\((['"]?)(.+?)\1\)/g, function(str, quote, url) {
		var targetUrl = url.trim();
		if (config.validUrl(targetUrl)) {
			var absUrl = absoluteUrl(targetUrl, file.path, config.baseDir);
